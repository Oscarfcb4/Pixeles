...
struct ResourceManager{
    ...
    R_Shader* getShader(std::string name, std::string vs, std::string fs, bool isFile = false);
    ...
    template <class ResourceType>
    ResourceType* getResource(std::string name, std::string vs, std::string fs, bool isFile = false){
        std::vector<std::shared_ptr<R_Resource>>::iterator it = std::find_if(resources.begin(), 
            resources.end(), [&] (std::shared_ptr<R_Resource> r){
            if(r->name == name){
                return true;
        ...
        if(it == resources.end()){
            ...
            auto newRec = std::make_shared<ResourceType>(vs.c_str(), fs.c_str(), isFile);
            newRec->setName(name);
            newRec->setScene(scene);
            resources.push_back(std::static_pointer_cast<R_Resource>(newRec));
            return newRec.get();
