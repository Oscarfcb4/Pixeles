...
struct E_Camera: virtual public E_Entity{
    ...
    Vec3 getFront() { return Vec3(front); };
    Vec3 getUp() { return Vec3(up); };
    Vec3 getRight() { return Vec3(right); };
    Vec3 getWorldUp() { return Vec3(worldUp); };

    void setPosition(Vec3 position) { this->position = glm::vec3(position.x, position.y, position.z); };
    void setFront(Vec3 front) { this->front = glm::vec3(front.x, front.y, front.z); };
    void setUp(Vec3 up) { this->up = glm::vec3(up.x, up.y, up.z); };
    void setRight(Vec3 right) { this->right = glm::vec3(right.x, right.y, right.z); };
    void setWorldUp(Vec3 worldUp) { this->worldUp = glm::vec3(worldUp.x, worldUp.y, worldUp.z); };
