...
struct TNode{
    ...
    void renderShadows(glm::mat4 cumulativeMatrix, _Shader* shader);
...
inline void translate(ResourceType* rs, Vec3 trans){
    rs->node->translate(trans);
}

template <class ResourceType>
inline void scale(ResourceType* rs, Vec3 scale){
    rs->node->scale(scale);
}

template <class ResourceType>
inline void rotate(ResourceType* rs, Vec4 rot){
    rs->node->rotate(rot);
}