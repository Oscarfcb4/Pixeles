...
struct TAnimationModel: virtual public TEntity{
	...
	void draw(glm::mat4 matrix = glm::mat4(1.0f), TCamera* principalCamera = nullptr) override{
		...
		if(shader && model && principalCamera){
			...
			actualShader.setBool("anim", true);
			auto trans = model->getAnimator().getBoneMatrices();
			for(unsigned int i = 0; i < trans.size(); i+=1){
				actualShader.setMat4("finalBonesMatrices[" + std::to_string(i) + "]", trans[i]);
			}
    ...
    void playAnimation(Animation* anim){
        model->getAnimator().play(anim);
    }

	void updateAnimation(float delta){
        model->getAnimator().update(delta);
    }
